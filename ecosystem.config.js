module.exports = {
  ...
  deploy: {
    production: {
      ...
      "post-deploy": [
        "cp -n /var/www/shared/.env /var/www/api-backend/current/backend/.env || true",
        "cd /var/www/api-backend/current/backend",
        "npm ci",
        "npm run build",
        "npx prisma generate",
        "npx prisma migrate deploy || true",
        "npm prune --omit=dev",
        "cd /var/www/api-backend/current",
        "pm2 startOrReload /var/www/api-backend/current/ecosystem.config.js --env production"
      ].join(" && ")
    }
  }
}
